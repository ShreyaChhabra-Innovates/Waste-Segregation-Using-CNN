...
train_biodegradable_dir = '/content/drive/MyDrive/Colab Notebooks/Waste_Segregation/Dataset/train/biodegradable'
train_non_biodegradable_dir = '/content/drive/MyDrive/Colab Notebooks/Waste_Segregation/Dataset/train/non_biodegradable'

val_biodegradable_dir = '/content/drive/MyDrive/Colab Notebooks/Waste_Segregation/Dataset/val/biodegradable'
val_non_biodegradable_dir = '/content/drive/MyDrive/Colab Notebooks/Waste_Segregation/Dataset/val/non_biodegradable'
...
class_counts = {}
for path in train_dataset_bio.imgs:
    class_name = os.path.basename(os.path.dirname(path[0]))
    class_counts[class_name] = class_counts.get(class_name, 0) + 1
for path in train_dataset_non_bio.imgs:
    class_name = os.path.basename(os.path.dirname(path[0]))
    class_counts[class_name] = class_counts.get(class_name, 0) + 1

print("\nClass Counts:", class_counts)

# Reorder class counts to match the order of class_names
sorted_class_counts = [class_counts[name] for name in class_names]
print("Sorted Class Counts:", sorted_class_counts)

total_images = sum(sorted_class_counts)
class_weights = [total_images / count for count in sorted_class_counts]
...
import matplotlib.pyplot as plt
import numpy as np
# Adjust the lists and the x-axis range to start from the second epoch
epochs_to_plot = range(2, num_epochs + 1)
train_accuracies_slice = train_accuracies[1:]
val_accuracies_slice = val_accuracies[1:]

plt.figure(figsize=(10, 5))
plt.plot(epochs_to_plot, train_accuracies_slice, label='Training Accuracy', marker='o')
for i, (x, y) in enumerate(zip(epochs_to_plot, train_accuracies_slice)):
    plt.text(x, y + 1.5, f'{y:.2f}', ha='center', va='bottom', fontsize=8)

plt.plot(epochs_to_plot, val_accuracies_slice, label='Validation Accuracy', marker='o')
for i, (x, y) in enumerate(zip(epochs_to_plot, val_accuracies_slice)):
    plt.text(x, y - 1.5, f'{y:.2f}', ha='center', va='top', fontsize=8)

plt.title('Training vs. Validation Accuracy (Epochs 2-10)')
plt.xlabel('Epoch')
plt.ylabel('Accuracy (%)')
...
torch.save(model.state_dict(), '/content/drive/MyDrive/Colab Notebooks/Waste_Segregation/resnet50_waste_new_model.pth')
print("Model saved as resnet50_waste_new_model.pth")